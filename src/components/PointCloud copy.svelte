<script lang="ts">
  ...
  onMount(() => {
    ...
    pointCloudListener = subscribeToPointCloud(ros, (message: any) => {
      updatePointsFromLaserScan(message);
    });
    tfListener = subscribeToTF(ros, handleTFMessage);
    return () => {
...
{#each points as point}
  <T.Mesh position={[point.x, point.y, point.z]}>
    ...
    <T.MeshStandardMaterial color={0x46affa} transparent opacity={0.3} />
  </T.Mesh>
{/each}

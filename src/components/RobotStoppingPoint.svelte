<script lang="ts">
  ...
  import { connectToROS, subscribeToTF } from "../lib/ros/subscriptions";
  ...
  onMount(() => {
    ...
    const ros = connectToROS("ws://3.143.219.5:9090"); // 18.222.63.241

    let mapToOdom: any = null;
    let odomToBase: any = null;

    const tfListener = subscribeToTF(ros, (msg) => {
